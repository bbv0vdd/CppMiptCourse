...
namespace FileStorage
{
    ...
    void write(const std::string& path, const std::vector<TestCase>& cases)
    {
    ...
    std::vector<TestCase> read(const std::string& path)
    {
        ...
        while (std::getline(file >> std::ws, expr))
        {
            double val = 0.0;
            if (!(file >> val))
                throw std::runtime_error("bad test file");

            cases.push_back({expr, val});
        }
...
namespace CalculatorV1
{
    ...
    void test()
    {
        const auto path = "calc_v1_test.data";

        std::vector<FileStorage::TestCase> cases = {
            {"2 + 3 * 4", 14.0},
            {"20 % 6", 2.0},
            {"2 ^ 3 ^ 2", 512.0},
            {"5!", 120.0},
        ...
        FileStorage::write(path, cases);
        auto loaded = FileStorage::read(path);
        std::filesystem::remove(path);

        if (loaded.size() != cases.size())
            throw std::runtime_error("bad test count");

        Engine calc;

        for (const auto& tc : loaded)
            requireClose(calc.evaluate(tc.expression), tc.result);
    }

    void demo()
    {
        const auto path = "calc_v1_demo.data";

        std::vector<FileStorage::TestCase> examples = {
            {"2 + 3 * 4", 14.0},
            {"20 % 6", 2.0},
            {"2 ^ 3 ^ 2", 512.0},
            {"5!", 120.0},
        ...
        FileStorage::write(path, examples);
        auto loaded = FileStorage::read(path);
        std::filesystem::remove(path);
        ...
        for (const auto& ex : loaded)
            std::print("{} = {}\n", ex.expression, calc.evaluate(ex.expression));
    }
}
...
namespace CalculatorV2
{
    ...
    void test()
    {
        const auto path = "calc_v2_test.data";
        ...
        for (const auto& tc : loaded)
            CalculatorV1::requireClose(parse(tc.expression), tc.result);
    }

    void demo()
    {
        const auto path = "calc_v2_demo.data";
        ...
        for (const auto& ex : loaded)
            std::print("{} = {}\n", ex.expression, parse(ex.expression));
    }
}
